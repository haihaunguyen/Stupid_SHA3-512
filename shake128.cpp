...
void showS(uint64_t A[5][5]){
    for (int x =0; x<4; x++){
        for (int y = 0; y<5; y++){
            cout<< hex <<setfill('0')<<setw(16)<<A[y][x];
        }
    }
     cout<< hex <<setfill('0')<<setw(16)<<A[0][4];
}
...
void Padding(uint8_t input[], int &in_len, int &absorb_times){
    absorb_times = in_len / 168;
    int add_last = in_len % 168;
    if (168 - add_last == 2) {
        input[in_len] = 0x1f;
        input[in_len+1] = 0x80;
    }
    ...
    input[in_len] = 0x1f;
    for (int i = 1; i < (167 - add_last); i++){         
        input[in_len+i] = 0x00;      
        //printf("%02x", input[in_len+i]);
    }   
    input[in_len+(167 - add_last)] = 0x80; 
    in_len = in_len + (168 - add_last);
    }
...
void change_type_S(uint64_t S[5][5]){
    for (int i = 0; i< 5; i++){
        for (int j = 0; j <5; j++) EndianSwap(S[i][j]);
    }
}
...
int main(){
    ...
    cout<<"Length of data: ";
    cin>>len;
    cout<<"Number of bit you wanna get: ";
    cin>>number_hash;
    ...
    //squeezing phase
    number_hash = number_hash / 8;
    number_hash = (number_hash / 168);
